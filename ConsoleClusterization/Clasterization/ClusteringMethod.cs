...
namespace Clusters.Clasterization
{
    public abstract class ClusteringMethod<T>
    {
        ...
        public ClusteringMethod(int count)
        {
            minCount = count;
        }

        public virtual List<List<T>> clusterize(List<T> set, Metrica<T> metrica)
        {
            this.initialize(set, metrica);
            while (operatingClusters.Count > this.minCount)
            {
        ...
        private void initialize(List<T> set, Metrica<T> metrica)
        {
            this.set = set;
